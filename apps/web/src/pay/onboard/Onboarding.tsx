 ...
 *   4. CardOrder        — customise the card skin + "order" it (shipped home).
 *   5. VoiceOnboarding  — your chosen companion interviews you by voice and
 *                         fills the persona (save_persona), then completes.
 * The name captured at step 3 is threaded into steps 4-5 so it's never re-asked.
 */
...
export function Onboarding({ role, onDone }: { role: 'parent' | 'kid'; onDone: () => void }) {
  const [stage, setStage] = useState<'intro' | 'choose' | 'name' | 'card' | 'voice'>('intro')
  ...
  if (stage === 'name') {
    return <NameCard role={role} onDone={(n) => { setName(n); setStage('card') }} />
  }
  if (stage === 'card') {
    return <CardOrder role={role} name={name} onDone={() => setStage('voice')} />
  }
