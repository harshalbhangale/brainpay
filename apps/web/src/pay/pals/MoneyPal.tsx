...
import { Home as HomeIcon, Receipt, CreditCard, Plus, Users, ListChecks, Camera } from 'lucide-react'
...
const PARENT_TABS: { key: TabKey; label: string; Icon: typeof HomeIcon }[] = [
  ...
  { key: 'cards', label: 'Card', Icon: CreditCard },
]
  ...
  return (
    <>
      <div key={tab} className="pv-pal-enter flex min-h-0 flex-1 flex-col">
        {tab === 'home' && <Dashboard go={setTab} goPal={goPal} onTopUp={() => setTopUp(true)} />}
        {tab === 'family' && !isKid && <Family />}
        {tab === 'chores' && isKid && <KidChores />}
        {tab === 'activity' && <Activity />}
        {tab === 'cards' && <CardScreen />}
      </div>

      {isKid ? (
        <BottomNav active={tab} onChange={setTab} onCenter={() => setVerify(true)} centerIcon={Camera} tabs={KID_TABS} />
      ) : (
        <BottomNav active={tab} onChange={setTab} onCenter={() => setTopUp(true)} centerIcon={Plus} tabs={PARENT_TABS} />
      )}
