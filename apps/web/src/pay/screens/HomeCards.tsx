...
import { ChevronRight, ListChecks, Receipt, Plus, CreditCard, MapPin, Target, Wallet, GraduationCap, PieChart } from 'lucide-react'
...
export function HomeCards({ onAsk }: { onAsk: (text: string) => void }) {
  ...
  return (
    <div className="space-y-3">
      ...
      {/* Spend insights — live breakdown from the ledger */}
      <button onClick={() => open('insights')} className="pv-press flex w-full items-center gap-3 rounded-[var(--pv-r-lg)] p-4 text-left" style={{ background: 'var(--pv-surface)', boxShadow: 'var(--pv-shadow-sm)' }}>
        <IconBadge Icon={PieChart} tile="peach" size={40} />
        <div className="min-w-0 flex-1">
          <div className="pv-title">{isKid ? 'Your spending' : `${subjectName === 'the family' ? "The family's" : subjectName + "'s"} spending`}</div>
          <div className="text-sm font-semibold" style={{ color: 'var(--pv-ink-3)' }}>Where the money goes — in & out</div>
        </div>
        <ChevronRight size={18} style={{ color: 'var(--pv-ink-3)' }} />
      </button>
