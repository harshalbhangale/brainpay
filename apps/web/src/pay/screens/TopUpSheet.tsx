...
export function TopUpSheet({ onClose, presetKidId }: { onClose: () => void; presetKidId?: string }) {
  ...
  const showFooter = !done && kids.length > 0

  return (
    <BottomSheet
      onClose={onClose}
      title={done ? undefined : 'Add money'}
      showClose={!done}
      footer={showFooter ? (
        <Button variant="primary" size="lg" full disabled={!kid || amount <= 0 || give.isPending} onClick={submit}>
    ...
    >
      {done ? (
        ...
        <p className="pv-body py-6 text-center" style={{ color: 'var(--pv-ink-2)' }}>
        ...
        <>
          ...
          {!presetKidId && (
            <>
              <p className="pv-label mb-2">To</p>
